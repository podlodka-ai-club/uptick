...
- `DecisionModel` — получение одного решения по схеме, объявленной средой;
- `Environment` — фиксированные стартовые инструкции и схема инструментов, запуск мира,
  проекция наблюдаемого состояния и выполнение типизированных действий;
...
Исходники организованы по ответственности: `environment/` задаёт контракт среды,
`decisions/` — универсальную инструкцию и контекст, `simulator/` — SRE-инструменты,
...
его можно использовать без загрузки CLI и конкретного провайдера. Схема решения
обязательна и передаётся средой. В v2 стартовый текст берётся из фактического
`commands_markdown` сервера после удаления секретов; фиксируется до создания
модели и не меняется в ходе run. Детали и проверки:
[`ENVIRONMENT_BOUNDARY.md`](docs/agent-memory-design/ENVIRONMENT_BOUNDARY.md).
...
Короткий рабочий контекст последних шести действий и публичное состояние,
которое проецирует адаптер среды, передаются модели всегда.
